...
filename <- "D:/Alex/COPD/SC Outcomes 2018/Logs/COPD_SC_Outcomes_2018_analysis_log"
filedate <- Sys.Date()
...
unadjORdied30
unadjORdied30 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
adjORdied30
adjORdied30 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
unadjORdied90
unadjORdied90 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
adjORdied90
adjORdied90 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
unadjORread30
unadjORread30 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
adjORread30 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
unadjORread90
unadjORread90 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
adjORread90 %>% as.data.frame() %>% unite(CI, -est, sep = " to ") %>% CPwithrn()
...
# So painful getting this graph together.
# Absolute lottery knowing whether the settings should go in the ggsurvplot backets, in the theme brackets, or 
# added on by a '+' at the end.
